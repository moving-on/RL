...
class Memory(object):
    ...
    def sample(self, transition):
        ...
        for i in range(n):
            ...
            ISWeights[i, 0] = np.power(prob/in_prob, -self.beta)
...
class DQNPrioritizedReplay:
    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            ...
            b1 = tf.get_variable('b1', [1, n_l1], initializer=b_initializer, collections=c_name, trainable=trainable)
            l1 = tf.nn.relu(tf.matmul(s, w1) + b1)

            w2 = tf.get_variable('w2', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names, trainable=trainable)
            b2 = tf.get_variable('b1', [1, self.n_actions], initializer=b_initializer, collections=c_name, trainable=trainable)
        ...
        with tf.variable_scope("target_net"):
            c_names, n_l1, w_initializer, b_initializer = 
                ['target_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 20,
                tf.random_normal_initializer(0., 0.3),
                tf.constant_initializer(0.1)
    ...
    def learn(self):
        if self.learn_step_counter % self.replace_target_iter == 0:
            self.sess.run(self.repalce_target_op)
            print('\ntarget params replace\n')
        if self.prioritized:
        ...
        if self.prioritied:
            _, abs_errors, self.cost = self.sess.run([self.train_op, self.abs_error, self.loss], 
                                                     feed_dict={self.s:batch_memory[:, :self.n_features],
                                                                self.q_target:q_target,
                                                                self.ISWeights:ISWeights})
            self.memory.batch_update(tree_idx, abs_erros)
        else:
