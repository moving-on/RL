...
class NetWork(object):
    def __init__(self, n_actions, n_features, name=None, learning_rate=0.01):
...
class DeepQNetwork(object):
    ...
    def build_net(self):
        ...
        with tf.variable_scope("eval_net"):
            ...
            V1 = tf.layers.dense(l1, 1)
            A1 = tf.layers.dense(l1, self.n_actions)
            self.q_eval = (A1 - tf.reduce_mean(A1, axis=1, keep_dims=True)) + V1

        with tf.variable_scope("target_net"):
            l2 = tf.layers.dense(self.state, 10, tf.nn.relu)
            V2 = tf.layers.dense(l2, 1)
            A2 = tf.layers.dense(l2, self.n_actions)
            self.q_next = (A2 - tf.reduce_mean(A2, axis=1, keep_dims=True)) + V2
    ...
    def plot_cost(self):
        ...
        plt.cla
        print self.cost_his
        plt.plot(range(len(self.cost_his)), self.cost_his)
        plt.ioff()
        plt.show() 
